...
import { FileText, Calendar, User, Eye, Clock, Search, CheckCircle } from 'lucide-react';
...
const DispensedMedicines = ({ onViewPrescription }) => {
  ...
  try {
    ...
    const response = await axios.get(`${API_BASE_URL}/pharmacy/prescriptions/dispensed`, {
    ...
    const serverMsg = err.response?.data?.message || err.message || 'Failed to fetch dispensed records';
    setError(serverMsg);
  ...
  const filteredRecords = dispensedRecords.filter(record =>
    ...
    record.patient?.medTrackId?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    record.prescription?.diagnosis?.toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div className="space-y-4">
          {filteredRecords.map((record) => (
            <div
              ...
              onClick={() => onViewPrescription(record.prescription)}
            >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-4 text-sm text-gray-500 dark:text-gray-400">
                    <div className="flex items-center space-x-1">
                      ...
                      <span className="text-xs">({record.patient?.medTrackId})</span>
                    </div>
                    <div className="flex items-center space-x-1">
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <span>{record.prescription?.medicineNames?.length || 0} medicines</span>
                    </div>
                  </div>
